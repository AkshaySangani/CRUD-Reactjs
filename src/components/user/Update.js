...
class Update extends React.Component {
  ...
  onSubmit = async (e) => {
    e.preventDefault();
    await axios.put(`http://localhost:3003/users/${this.state.id}`, this.state.user);
  ...
  loadUser = async () => {
    const {data} = await axios.get(`http://localhost:3003/users/${this.state.id}`);
    this.setState({ user: { ...this.state.user, ...data } });
  };
  render() {
    const { name="", username="", email="", phone="", website="", gender="" } = this.state.user;
    return (
      <div className="container">
        <div className="w-75 mx-auto shadow p-5">
          <h2 className="text-center mb-4">Edit A User</h2>
          <form onSubmit={e => this.onSubmit(e)}>
            ...
            <div className="form-group d-flex">
              <label>Gender:</label>
              <div className="form-check">
                <input className="form-check-input" type="radio" value={"male"} checked={gender === "male" ? true : false} name="gender" id="flexRadioDefault1" onChange={e => this.onInputChange(e)} />
                <label className="form-check-label" htmlFor="flexRadioDefault1">
                  male
                </label>
              </div>
              <div className="form-check">
                <input className="form-check-input" type="radio" value={"female"} name="gender" checked={gender === "female" ? true : false} id="flexRadioDefault2" onChange={e => this.onInputChange(e)} />
                <label className="form-check-label" htmlFor="flexRadioDefault2">
                  female
                </label>
              </div>
